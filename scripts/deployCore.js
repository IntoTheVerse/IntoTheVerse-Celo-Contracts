...
async function main() {
  ...
  console.log(` - Deploying Mock NFT Token...`)
  const nft = await MockNFT.deploy('NFT TKN', 'NFT')
  await nft.deployed()
  deploymentDetails['NFT'] = {
    contractNameForAbi: 'MockNFT',
    abi: nft.interface,
    address: nft.address,
  }
  ...
  const treeContract = await TreeContract.deploy(
    ...
    '0xF194afDf50B03e69Bd7D057c1Aa9e10c9954E4C9', // Wrapped Native token
  ...
  console.log(` - Deploying MarketplaceRetirementCertificateEscrow...`)
  const marketplaceRetirementCertificateEscrow = await MarketplaceRetirementCertificateEscrow.deploy();
  await marketplaceRetirementCertificateEscrow.deployed()
  deploymentDetails['MarketplaceRetirementCertificateEscrow'] = {
    contractNameForAbi: 'MarketplaceRetirementCertificateEscrow',
    abi: marketplaceRetirementCertificateEscrow.interface,
    address: marketplaceRetirementCertificateEscrow.address,
  }
  ...
  const marketplace = await Marketplace.deploy(
    ...
    "0xF0a5bF1336372FdBc2C877bCcb03310D85e0BF81", // TC02
    "0xd4de140b1064a40a00cd756ab812181e7e14da04", // TC02 retirement certificate
    marketplaceRetirementCertificateEscrow.address,
    "0xF194afDf50B03e69Bd7D057c1Aa9e10c9954E4C9", // Wrapped Native token
  )
  await marketplace.deployed()
  deploymentDetails['Marketplace'] = {
    contractNameForAbi: 'NftMarketplace',
    abi: marketplace.interface,
    address: marketplace.address
  }
  console.log()

  console.log(`Linking Protocol Core Contracts...`)
  await treeContract.setCost(parseUnits('1', 'gwei'))
  await treeContract.setGreenDonationContract(await greenDonation.address)
  await treeContract.setRetirementCertificateEscrow(await retirementCertificateEscrow.address)
  ...
  await marketplaceRetirementCertificateEscrow.setNFTMarketplace(marketplace.address)
  await marketplaceRetirementCertificateEscrow.setRetirementCertificate("0xd4de140b1064a40a00cd756ab812181e7e14da04")
  console.log()
  ...
  console.log('Adding liquidity to ubeswap pairs...')
  const tc02 = await ethers.getContractAt('ERC20', '0xF0a5bF1336372FdBc2C877bCcb03310D85e0BF81')
  const celoNative = await ethers.getContractAt('ERC20', '0xF194afDf50B03e69Bd7D057c1Aa9e10c9954E4C9')
  ...
  const celoNativeApprovalTx = await celoNative.approve(ubeswapRouter.address, parseEther('10000000'))
  await celoNativeApprovalTx.wait(1)
  console.log(' - Adding liquidity')
  let tc02Balance = await tc02.balanceOf(await deployer.getAddress())
  const rewardTokenBalance = await rewardToken.balanceOf(await deployer.getAddress())
  const celoNativeBalance = await celoNative.balanceOf(await deployer.getAddress())
  console.log(' - Awaiting successful addLiquidity of RWD and TC02 ...')
  let addLiquidityTx = await ubeswapRouter.addLiquidity(
    rewardToken.address,
    tc02.address,
    rewardTokenBalance,
    tc02Balance.div(2),
  ...
  tc02Balance = await tc02.balanceOf(await deployer.getAddress())
  console.log(' - Awaiting successful addLiquidity of Wrapped native token and TC02 ...')
  addLiquidityTx = await ubeswapRouter.addLiquidity(
    celoNative.address,
    tc02.address,
    celoNativeBalance.div(3),
    tc02Balance.div(2),
    celoNativeBalance.div(3).div(2),
    tc02Balance.div(2).div(2),
    await deployer.getAddress(),
    Math.floor(Date.now() / 1000) + 24 * 60 * 60
  )
